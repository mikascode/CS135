...
(define (getjusty R)
      (if(null? R)
      '()
       (append (list (car(cdr(car R)))) (getjusty (cdr R)))))

(define (getjustx S)
      (if(null? S)
      '()
       (append (list (car(car S))) (getjustx (cdr S)))))
(define (make-set L)
  (cond [(null? L) '()]
...
(define (get-vertices G)
 (usort(make-set (append (getjusty G) (getjustx G)))))
...
(define (helper3 G v)
  ...
  (if (equal? (car G) v)
     ...
     (+ 0  (helper3 (cdr G) v)))))
(define (degree G v)
  (helper3 (append (getjustx G) (getjusty G)) v))
...
(define (evenhelper G o)
  (if (null? G )
      '()
      (append (list (degree o (car (get-vertices G)))) (evenhelper  (cdr G) o))))
...
(define (all-even? G)
  (if ( = (length (evenhelper G G)) (even (evenhelper G G)))
      #t
      #f))
...
(define (relate x R)
  ( if (null? R)
       '()
       (if (= (car (car R)) x)
       (make-set (cons (car(cdr(car R)))  (relate x (cdr R))))
...
(define (relate2 x R)
  ( if (null? R)
       '()
       (if (= (car (cdr (car R))) x)
       (make-set (cons (car(car R)) (relate2 x (cdr R))))

        (relate2 x (cdr R)))))

(define (adjacent G v)
 (usort (append (relate v G) (relate2 v G))))
...
(define (helper101 G x)
 (usort (dfs G x)))

  (define (dfshelp G o)
  (if (null? G )
      '()
        (append (list (helper101 o (car (get-vertices G)))) (dfshelp  (cdr G) o))))


(define (helper60 G o )
  (if (null? G)
      0
      (if ( = (length (car G)) (length (get-vertices o)))
          (+ 1 (helper60 (cdr G) o))
...
(define (connected? G)
  (if ( = (length (dfshelp G G)) (helper60 (dfshelp G G)G))
      #t
      #f))
