...
;; 2 PTS
(define (yoda words)
   (append  (list (car(cdr(cdr words)))) (list (car words)) (list(car(cdr words)))))
...
(define (pig-latin word)
  (append   (cdr(car(list word) ))  (list(car(car(list word) ))) (list 'a 'y)))
...
(append  ( list (/ (- (* -1 b )(sqrt (- (* b b ) (* 4 a c)))) (* 2 a )))     ( list (/ (+ (* -1 b )(sqrt (- (* b b ) (* 4 a c)))) (* 2 a ))) ))
...
;; 2 PTS
(define (rotate c)
  ( append (list c ) (list (list (car(cdr c)) (* -1 (car c))))  (list (list (* -1 (car c )) (* -1 (car(cdr c )))))  ( list (list (* -1 (car(cdr c))) (car c)))))
...
(define (combine-courses s1 s2)
  (append    (car (list(car(cdr( cdr (cdr(cdr (cdr s1))))))))    ( car (list(car(cdr( cdr (cdr(cdr (cdr s2))))))))))