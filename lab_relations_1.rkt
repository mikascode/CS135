...
(define (id n)
 (if (= n 0) '()
   ( make-set (append (list (append (list n) (list n)))  (id (- n 1))))))
...
(define (reflexive? n R)
 (if (= (length (intersection (id n) R)) n )
     #t
     #f ))
...
;; 2 PTS
(define (symmetric? R)
  (if ( = (length (intersection R (converse R))) ( length R))
      #t
      #f))
...
(define (relate x R)
  ( if (null? R)
       '()
       (if (= (car (car R)) x)
       (make-set (append (cdr(car R)) (relate x (cdr R))))
...
(define (helper2 R)
  (cardinality (helper R)))

(define (helper R)
  (if (null? R) '()
   (append (cdr(car R)) (helper (cdr R)))))

(define (injective? R)
  (if   ( = (helper2 R) (length  (helper R)))
        #t
        #f))
