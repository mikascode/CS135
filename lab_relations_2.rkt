...
(define (composeHelper S R)

  (if (null? R)
      '()

    (if (equal? (getFirst S) (getSecond R))
        (cons (list (getFirst R) (getSecond S)) (composeHelper S (cdr R)))

        (composeHelper S (cdr R)))))
...
(define (compose S R)
  (cond
    [(null? S) '()]
    [else (make-set (append (composeHelper S R) (compose (cdr S) R)))]
    )
)
...
(define (gety S R)
  (if (null? S)
      ...
      (append (relate (car(cdr(car R))) S) (gety S (cdr R))))))
...
(define (unite S R)
  (intersection (getjusty R) (getjustx S)))
...
;; 5 PTS
(define (powerHelp k R acc)
  (if (= k 1)
      acc
 (powerHelp (- k 1) R (compose R acc))))
(define (power k R)
  (if (= k 0) '()
  (powerHelp k R R)))
...
(define (helpt R k x)
  (if (= k 0)
      x
      (helpt R (- k 1) (union (compose R x) x))))
(define (transitive-closure R)
  (helpt R (cardinality R) R))
...
#| Implement "equivalence?" t`o accept a positive integer n and a relation R
...
(define (equivalence? n R)
  ...
  ( if (and (reflexive? n R) (transitive? R) (symmetric? R))
       #t
       #f 
  )) 
