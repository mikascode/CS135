...
(define (generate p q e)
  (append (list (list ( * p q) e )) (list (list (* p q) (mul-inv e (lcm (- p 1) (- q 1)) )))))
...
(if (= e 0) 
    1
    (if (and (> e 0) (not (= 0 (modulo e 2))))
     (modulo  (* b  (mod-exp b (- e 1) m)) m)
...
(define (helper2 plaintext key)
(if (null? plaintext)
    '()
    (append (list (mod-exp (car plaintext)  (car(cdr key)) (car key)))  (helper2 (cdr  plaintext) key))))
 
(define (encrypt plaintext key)
  (helper2 (helper plaintext) key))
...
;; 6 PTS
(define (helper3 c key)
  (integer->char (mod-exp c (car(cdr key)) (car key))))
(define (decrypt ciphertext key)
  (list->string (map (lambda (x)(helper3 x key)) ciphertext)))
