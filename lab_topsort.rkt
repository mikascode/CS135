...
(define (helper3 G S)
  (if (null? S)
      '()
      (if ( = (indegree G (car S)) 0)
          (append (list (car S) (helper3  G (cdr S) )))
          (helper3 G (cdr S)))))
       
(define (find-top G)
  (if (= (length (helper3 (get-edges G) (get-vertices G))) 0 )
      (topsort-error)
      (car (helper3 (get-edges G) (get-vertices G)))))
